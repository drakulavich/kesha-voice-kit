...
export interface CollisionPlan {
  edges: Array<{
    source: "claim" | "pull-request" | "worktree";
    issue?: number;
    pr?: number;
    path: string;
  }>;
...
function record(value: unknown, source: string): JsonRecord {
  if (!isRecord(value))
    throw new CoordinationError(`${source} must be an object`);
  return value;
}
function string(value: unknown, source: string): string {
  if (typeof value !== "string" || value.length === 0)
    throw new CoordinationError(`${source} must be a non-empty string`);
  return value;
}
function number(value: unknown, source: string): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value))
    throw new CoordinationError(`${source} must be an integer`);
  return value;
}
function array(value: unknown, source: string): unknown[] {
  if (!Array.isArray(value))
    throw new CoordinationError(`${source} must be an array`);
  return value;
}
export function timestamp(value: unknown, source: string): string {
  const text = string(value, source);
  if (Number.isNaN(Date.parse(text)))
    throw new CoordinationError(`${source} must be an ISO timestamp`);
  return text;
}
...
function ttl(value: unknown, source: string): number {
  const seconds = number(value, source);
  if (seconds < 1 || seconds > 86_400)
    throw new CoordinationError(
      `${source} must be between 1 and 86400 seconds`,
    );
  return seconds;
}
export function parseClaimManifest(
  value: unknown,
  ...
  if (issue < 1)
    throw new CoordinationError(`${source}.issue must be positive`);
  const paths = array(manifest.paths, `${source}.paths`).map((path, index) =>
    normalizedPath(path, `${source}.paths[${index}]`),
  );
  if (paths.length === 0)
    throw new CoordinationError(`${source}.paths must not be empty`);
  return {
    version: 1,
    issue,
    holder: string(manifest.holder, `${source}.holder`),
    paths: [...new Set(paths)].sort(),
    ttlSeconds: ttl(manifest.ttlSeconds, `${source}.ttlSeconds`),
  };
}
...
export function parseClaimMarker(body: string): ClaimMarker | null {
  ...
  try {
    ...
    const expiresAt =
      marker.expiresAt === null || marker.expiresAt === undefined
        ? null
        : timestamp(marker.expiresAt, "claim marker.expiresAt");
    if (action === "claim" && expiresAt === null) return null;
    return {
      version: 1,
      action,
      manifest: parseClaimManifest(marker.manifest, "claim marker.manifest"),
      expiresAt,
    };
  } catch {
    return null;
...
function overlap(left: string[], right: string[]): string | null {
...
export function claimOrder(left: ClaimRecord, right: ClaimRecord): number {
  return left.createdAt.localeCompare(right.createdAt) || left.id - right.id;
}
export function isLiveClaim(record: ClaimRecord, now: Date): boolean {
  return (
...
export function evaluateCollisionPlan(
  manifest: ClaimManifest,
  facts: {
    claims: ClaimRecord[];
    pullRequests: Array<{
  ...
  for (const pullRequest of facts.pullRequests) {
    const path = overlap(manifest.paths, pullRequest.files);
    if (path !== null)
      (pullRequest.closingIssueNumbers.includes(manifest.issue)
        ? self
        : edges
      ).push({ source: "pull-request", pr: pullRequest.number, path });
  }
  for (const worktree of facts.worktrees) {
    const path = overlap(manifest.paths, worktree.files);
    if (path !== null)
      (issueNumberFromBranch(worktree.branch) === manifest.issue
        ? self
        : edges
  ...
  return {
    ...
    self,
    idempotent: own !== null,
    expiresAt: own?.marker.expiresAt ?? null,
    rejectedClaimIds: rejected.map((claim) => claim.id),
  };
}
...
function resource(value: string): string {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(value))
    throw new CoordinationError(
      "lease resource must use 1-64 letters, digits, dots, underscores, or hyphens",
    );
  return value;
}
function holder(value: string): string {
  if (value.length === 0 || value.length > 256)
    throw new CoordinationError(
      "lease holder must be a non-empty string up to 256 characters",
    );
  return value;
}
export function leaseDirectory(gitCommonDirectory: string): string {
  ...
  try {
    ...
    throw new CoordinationError("shared git common directory is missing");
  }
  if (stat.isSymbolicLink() || !stat.isDirectory())
    throw new CoordinationError(
      "shared git common directory must be a real directory",
    );
...
function rootState(root: string, create: boolean): string | null {
  ...
  let parent;
  try {
    parent = lstatSync(dirname(requested));
  } catch {
    throw new CoordinationError("lease root parent is missing");
  }
  if (parent.isSymbolicLink() || !parent.isDirectory())
    throw new CoordinationError("lease root parent must be a real directory");
  ...
  if (stat.isSymbolicLink())
    throw new CoordinationError("lease root must not be a symlink");
  if (!stat.isDirectory())
    throw new CoordinationError("lease root must be a directory");
  return requested;
}
function statePath(root: string, name: string): string {
  const path = resolve(root, `${resource(name)}.json`);
  ...
  if (
    ...
    throw new CoordinationError("lease state path escapes its root");
  return path;
}
function parseLease(value: unknown, source: string): LeaseState {
  const lease = record(value, source);
  if (lease.version !== 1)
    throw new CoordinationError(`${source}.version must equal 1`);
...
function readLease(root: string, name: string): LeaseState | null {
  const path = statePath(root, name);
  if (!existsSync(path)) return null;
  const stat = lstatSync(path);
  if (stat.isSymbolicLink())
    throw new CoordinationError("lease state must not be a symlink");
  if (!stat.isFile())
    throw new CoordinationError("lease state must be a regular file");
  let value;
  try {
    value = JSON.parse(readFileSync(path, "utf8"));
  } catch {
    throw new CoordinationError("malformed lease state");
  }
  const lease = parseLease(value, "lease state");
  if (lease.resource !== name)
    throw new CoordinationError("lease state resource does not match its path");
  return lease;
}
function live(lease: LeaseState, now: Date): boolean {
  return Date.parse(lease.expiresAt) > now.getTime();
}
function guard(
  root: string,
  name: string,
  operation: () => LeaseResult,
): LeaseResult {
  const path = join(root, `.${name}.lock`);
  const temporary = join(
    root,
    `.${name}.${process.pid}.${randomUUID()}.lock-tmp`,
  );
  try {
    writeFileSync(temporary, JSON.stringify({ version: 1, resource: name }), {
      encoding: "utf8",
      mode: 0o600,
      flag: "wx",
    });
    try {
      ...
      try {
        ...
        if (stat.isSymbolicLink())
          throw new CoordinationError(
            "lease operation guard must not be a symlink",
          );
      } catch (error) {
        if (error instanceof CoordinationError) throw error;
      }
      throw new CoordinationError("lease operation is already in progress");
    }
  } finally {
...
function assertNoGuard(root: string, name: string): void {
  const path = join(root, `.${name}.lock`);
  if (!existsSync(path)) return;
  const stat = lstatSync(path);
  if (stat.isSymbolicLink())
    throw new CoordinationError("lease operation guard must not be a symlink");
  if (!stat.isFile())
    throw new CoordinationError("lease operation guard must be a regular file");
  try {
    const value = record(
      JSON.parse(readFileSync(path, "utf8")),
      "lease operation guard",
    );
    if (value.version !== 1 || value.resource !== name)
      throw new CoordinationError("malformed lease operation guard");
  } catch (error) {
    if (error instanceof CoordinationError) throw error;
    throw new CoordinationError("malformed lease operation guard");
  }
...
export function statusLease(
  ...
  return lease === null
    ...
    : { state: live(lease, now) ? "held" : "expired", lease };
}
export function acquireLease(
  root: string,
  input: { resource: string; holder: string; ttlSeconds?: number },
  now = new Date(),
): LeaseResult {
  const name = resource(input.resource);
  const owner = holder(input.holder);
  const seconds = ttl(input.ttlSeconds ?? 600, "lease ttlSeconds");
  const safeRoot = rootState(root, true)!;
  try {
    return guard(safeRoot, name, () => {
      const path = statePath(safeRoot, name);
      const current = readLease(safeRoot, name);
      if (current && live(current, now))
        return current.holder === owner
          ? { state: "already-owned", lease: current }
          : { state: "refused", lease: current };
      ...
      try {
        ...
        try {
          linkSync(temporary, path);
          return { state: "acquired", lease, recovered };
        } catch (error) {
          if (
            !(error instanceof Error) ||
            !("code" in error) ||
            ...
            throw error;
          return { state: "refused", lease: readLease(safeRoot, name) };
        }
      } finally {
        if (existsSync(temporary)) unlinkSync(temporary);
      }
    ...
    if (
      ...
      error.message === "lease operation is already in progress"
    )
      return { state: "refused", lease: readLease(safeRoot, name) };
    throw error;
  }
...
export function releaseLease(
  root: string,
  input: { resource: string; holder: string },
  now = new Date(),
): LeaseResult {
  const name = resource(input.resource);
  const owner = holder(input.holder);
  ...
  try {
    return guard(safeRoot, name, () => {
      const path = statePath(safeRoot, name);
      const current = readLease(safeRoot, name);
      if (current === null || !live(current, now))
        return { state: "absent", lease: current };
      if (current.holder !== owner) return { state: "refused", lease: current };
      try {
        unlinkSync(path);
      } catch (error) {
        if (
      ...
      return { state: "released", lease: current };
    });
  } catch (error) {
    if (
      error instanceof CoordinationError &&
