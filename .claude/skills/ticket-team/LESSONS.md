...
| Date the incident evidence against the controls that closed it: the rate that matters is the one under the regime in force today | #1108 | 1 | The plan rejected "no change" solely on a tag ref that hosted three runs at three SHAs. Both instances are 2026-04-14 and 2026-04-15, days 1 and 2 of a 136-day window, and `6d9e6da` added TAG NAMES ARE ONE-USE to CLAUDE.md on 2026-04-15, the same day as the last one. The rate under the live regime is 0 in ~55 tags, not 5 in 96, and no stage asked. The first draft of this row also cited `dce4bef`'s uniqueness check as corroboration; it is gated `github.event_name == 'workflow_dispatch'` at `build-engine.yml:40` and never sees a push event, while every observed instance was push-path. A control on the wrong path is not evidence, and citing one makes a dated claim look stronger than it is |
