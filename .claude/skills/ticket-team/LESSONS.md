...
| what | caught at | earliest that could have | the change that would move it |
|---|---|---|---|
| **A closing keyword quoted as prose inside a commit message closed the audit.** #1105 was taken twice, twenty hours apart. `2026-08-28T16:11:46Z`, `commit_id: null` — #1109, reopened 68 seconds later by the maintainer. Then `2026-08-29T12:32:20Z` against `commit_id 27c0995`: the **protocol** PR #1107, whose body says only `Refs #1105`, whose squash concatenated 74 commit messages, one of which quotes the keyword twice while narrating the first mis-close. That commit, `8d3ac7a6`, is the one that added the closing-keyword rule. #1116, the audit's last item, merged 14m41s later and only "referenced" | nowhere — found at this retro, after the fact | hand-off on #1107, where the keyword check runs | Run the check against **both** channels — `gh pr view <N> --json body` and `git log <base>..<head> --format=%B` — and keep the keyword out of commit prose. Not because the two closes came from disjoint channels: #1109 carried the keyword in its body *and* in `b5bf9b7f`, and only its body was live because the squash message was trimmed to the title at merge. That is the reason for two checks — `squash_merge_commit_message` is `COMMIT_MESSAGES` here, so `git log` predicts the merge box rather than reading it, and the box is editable in both directions |
