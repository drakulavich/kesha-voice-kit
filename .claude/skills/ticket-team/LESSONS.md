...
| A closing keyword is checked for the right word, not for its presence, and in **both** channels a merge closes from — the pull request body and the squashed commit stream — because a squash concatenates every commit message and a keyword written as prose inside one is live | #1105 | 4 | `Closes #1105` shipped item one of a four-item audit and closed the other three on merge, including the one measured as the most valuable. The rule was then followed and failed: #1105 was closed a second time by `27c0995`, the protocol PR, whose body said `Refs #1105` while two of its 74 concatenated messages narrated the first mis-close and quoted the keyword doing it. `fired` does not move — the check ran and looked at the wrong text |
...
| what | caught at | earliest that could have | the change that would move it |
|---|---|---|---|
| **A closing keyword quoted as prose inside a commit message closed the audit.** #1105 was closed twice by two different routes twenty hours apart. `2026-08-28T16:11:46Z`, `commit_id: null` — #1109's pull request body, reopened 68 seconds later by the maintainer. Then `2026-08-29T12:32:20Z` against `commit_id 27c0995`: the **protocol** PR #1107, whose body says only `Refs #1105`, whose squash concatenated 74 commit messages, two of which quote the keyword while narrating the first mis-close — one of them this ledger's own account of it. #1116, the audit's last item, merged 14m41s later and only "referenced" | nowhere — found at this retro, after the fact | hand-off on #1107, where the keyword check runs | Run the check against **both** channels — `gh pr view <N> --json body` and `git log <base>..<head> --format=%B` — and keep the keyword out of commit prose. Written into §6 with the regex, because "check the body" is what was done here and it passed |
| **The per-assertion amendment shipped into two files and not the third.** `fd03896` added "both mutations" to `SKILL.md` alone, leaving `implementer.md` asking the planner for "the mutation", singular. #1117 then rewrote `implementer.md` and the ledger row to per-assertion and left `SKILL.md:223` reading "for each guard" — the planner asked for per-assertion rows while the judge holds the table to per-guard, the exact inverse of `fd03896`'s defect. PR #1119 is open against it | retro, pre-merge on #1117; by a second reader every previous time | authoring, in both cases | Upgrade the grep-the-other-files rule from advice to a step: the ledger-write step in §7 names `SKILL.md`, `teamlead.md`, `implementer.md` and `retro.md`, and records applied or explicitly declined per file. Four files, not three — `retro.md` is the one `40e75d0` found as the unfixed twin of the same bug |
...
| Grep the other files | 2 → **3** (`0fe6ab4`) | Keep and mechanised into §7. Never fired at authoring time in six instances |
...
`retro-audit-1105` proposed; the lead judged and wrote. Four things in this entry are the lead's
own wording rather than the retro's and were not judged by anyone else at write time: the
four-file list in §7 (the proposal said three and omitted `retro.md`), the both-channels form of
the closing-keyword check (the proposal said to replace the body check rather than add to it,
which the `commit_id: null` close of 2026-08-28 refutes), the third row above (the proposal's
version asserted a correlation the lead falsified), and the closing sentence of the fourth row.
