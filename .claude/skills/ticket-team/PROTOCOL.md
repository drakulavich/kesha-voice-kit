# ticket-team — the team lead's protocol

Everything here is addressed to the **team lead** spawned by the `ticket-team` skill; the skill's `SKILL.md` carries only the spawn instruction.
...
**Resolve the team's own four files — `SKILL.md`, `PROTOCOL.md`, `LESSONS.md`, `check-citations.ts` — by full
...
Which channel carries a result depends on the phase; pick the wrong one and you wait for a message
that cannot arrive. The mechanics:
...
**The path is the skill's own base directory, not the ticket tree's.** A worktree cut from an
older commit lacks the script while `.claude/skills/` still exists there, so a tree-relative path
resolves to nothing and a directory probe reports it installed. The base-directory path works in
both worlds.
...
None of that is a discovery here: CLAUDE.md's PR ETIQUETTE section states the same rule, and this
file carried only half of it.
...
change smuggled past its own separate-PR rule, invisible to the gates. Apply the same triage
...
instead of a re-read. Three rules, for every agent in the loop, each anchored to something this
loop paid for:
