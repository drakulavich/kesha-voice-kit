...
- Trigger: `release: published` (engine un-draft or published `v*-cli` marker), `push: main` (the alpha lane), and `workflow_dispatch` — with a `tag` to re-publish a release, without one to cut an alpha. Only `-cli` tags publish a CLI; a bare tag names the engine (#729).
