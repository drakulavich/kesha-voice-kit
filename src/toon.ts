...
/**
 * TOON (#138) encoding of an array of transcription results, or of the
 * `{ results, errors }` envelope when `errors` is passed (#839). Same data
 * shape as `formatJsonOutput`; lossless round-trip through
 * `@toon-format/toon`'s `decode()`.
 *
 ...
 */
export function formatToonOutput<T extends object, E extends object>(
  results: T[],
  errors?: E[],
): string {
