...
engine's own status (`4` if it broke the protocol on a clean exit), an error the
CLI raised before the spawn — a flag the installed build lacks, a missing engine,
a protocol mismatch, a backend this platform's release does not ship — maps its
code through the table above. For `record` and `install` an engine that exits
non-zero without reporting anything stays the operational `1`: there is no code
to relay, so the status says only that the run failed.
