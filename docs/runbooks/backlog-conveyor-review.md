...
Every Greptile requirement below applies **if the Greptile quota is available**.
Grok and the simplify pass are commissioned per PR, so if either is unavailable
nobody looked and the PR is blocked. Greptile reviews on its own trigger against
a third-party quota this repository does not control; its absence is a fact
about the vendor, not evidence about the change, and blocking on it would make
every conveyor PR unmergeable for the length of an outage.

The carve-out is narrow:

- It covers **Greptile only**. An unavailable or inconclusive Grok or simplify
  pass remains a hard blocker, exactly as `CLAUDE.md` states.
- Unavailability must be **recorded on the PR with the head SHA it applies to**,
  in the same comment that carries the rest of that head's evidence. A skipped
  gate nobody can see is worse than one that blocks.
- "Available but not finished yet" is not unavailable — that is a wait.
- Availability is judged per head. A quota that returns mid-review puts Greptile
  back in the required set for the next head.
...
pass. The fix pass pushes its own changes; the new head must repeat Grok
review, CI, and Greptile review when its quota is available (§0). A rejected
finding needs a PR comment explaining the evidence for rejecting it and
naming the current head SHA. On that SHA, a rejection closes the finding
only when its evidence is sufficient for a fresh agent to verify it. A clean
review means there are no outstanding *confirmed* P1/P2 findings on the
current head; it does not require deleting or rewriting the original review
comment.
...
- Greptile has covered that head, as required by `CLAUDE.md`, or its
  unavailability is recorded for that head (§0); and
...
new head restarts at step 1; it needs fresh Grok, CI, a new simplify pass, and
fresh Greptile when its quota is available (§0). If no capable simplify pass is
available or its result is inconclusive, record the blocker and leave the PR
without `merge-ready`.
...
§0 applies. Any subsequent push removes `merge-ready` and restarts this
runbook.