...
| Letter-name joiner | **Space** (`"вэ о зэ"`) | Vosk's BERT-prosody behaves better with space-separated tokens than dash-joined. С is position-dependent: "сэ" at start, "эс" elsewhere (#232 user listening). |
| Pronunciation gating | **Auto-expand only if length ≤ 2 OR adjacent same-type letter pair** | User listening test (#232) — alternating CVC/CVCV (ВОЗ, НАТО, ОПЕК) reads fine as words; only "broken" syllable shapes need spelling |
...
1. Iterate `input.chars()` with index.
2. Lowercase each char (`c.to_lowercase()`).
3. Position-dependent override: if index == 0 and char is 'с', use "сэ" instead of table lookup.
4. Look up in `LETTERS` table. If found, push the pronunciation string. If not found (non-Cyrillic, digit, punctuation), push the char unchanged.
...
2. For each token `t`:
   ...
   e. **`should_spell(core)` check (#232):** if `core` has strict CV/CVC alternation (no adjacent same-type letter pair) AND length > 2 → emit `t` unchanged. Such tokens read fine as words (ВОЗ → "воз", НАТО → "нато", ОПЕК → "опек").
   f. Otherwise emit `letter_table::expand_chars(core) + tail`.
3. Re-join tokens with the same whitespace runs they had.

`should_spell(core: &str) -> bool`: returns true when length ≤ 2 OR any adjacent pair of chars are both vowels or both consonants. Vowels: А Е Ё И О У Ы Э Ю Я.

`is_acronym_token(t: &str) -> bool` is the (b)+(c)+(e) check, exposed for test introspection.
...
- `expand_acronyms("ВОЗ")` == `"ВОЗ"` (CVC alternation → pass through as word).
- `expand_acronyms("КОТ")` == `"КОТ"` (CVC → pass through).
- `expand_acronyms("НАТО")` == `"НАТО"` (CVCV → pass through).
- `expand_acronyms("ОПЕК")` == `"ОПЕК"` (VCVC → pass through).
- `expand_acronyms("ОН пришёл")` == `"ОН пришёл"` (stop-list).
- `expand_acronyms("дом")` == `"дом"` (lowercase).
- `expand_acronyms("ВОЗа")` == `"ВОЗа"` (inflected — lowercase tail).
...
- Position-dependent letter-name forms only apply to С. Future user reports may extend to Ш, Ф, Ц, Ч etc.
- The rule treats acronyms with strict CV/CVC alternation as words (ВОЗ, КОТ, НАТО, ОПЕК); this is correct for these specific cases but may miss other acronyms that Russian convention spells (none surfaced in the test corpus). Add a per-acronym whitelist if such cases appear.
- Single-vowel multi-letter forms with non-clustered consonants are NOT covered by the rule (would be don't-spell). The 2-letter exception covers the most common short forms (ИП, ЕС, РФ).
