...
`resolveSayVoice` in the CLI, which sees the text, routes `ja`/`hi` text in its native script to an installed AVSpeech voice for that language when one exists (`ja-JP` prefers `Otoya`, male, and falls back to the first installed voice for that locale; `hi-IN` takes the first installed voice, which is `Lekha`, female, because macOS ships no male Hindi voice and `Rishi` is `en-IN` — a documented exception like Milena for `ru`), the same shape as the darwin `ru` route; romanized text keeps routing to the Kokoro voice. When no such voice is installed the engine's `E_SCRIPT_UNSUPPORTED` hint names the `macos-*` voices from `--list-voices` that carry the language. The routing table keeps an entry per language (#769: a missing entry silently drops `--voice`).
