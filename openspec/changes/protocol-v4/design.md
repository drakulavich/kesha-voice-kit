...
Every entry in `errors` carries an `origin` of `engine`, `cli` or `both`. `E_INPUT_NOT_FOUND`, `E_INVALID_ARG`, `E_UNSUPPORTED_PLATFORM` and `E_INTERNAL` are `both` — either side raises them (the CLI raises `E_UNSUPPORTED_PLATFORM` from the install pre-check below). `E_ENGINE_SPAWN`, `E_ENGINE_PROTOCOL` and `E_INSTALL_RACE` are `cli`: only the CLI can observe a binary that will not start, a protocol it does not speak, or a lost race for the install cache. With origins published, `docs/errors.md` is checked against `describe` rather than generated: `rust/tests/error_codes_docs.rs` holds the code sets equal in both directions, the capability pact compares category and retryability, `protocol-literals.test.ts` pins the codes the CLI names, and no TS registry needs a drift test. `warnings` publishes the `W_*` taxonomy the same way — code and title only, since a warning is never fatal and carries no category, retryability or origin — so a consumer can resolve every `code` a `warn` event carries.

The `install` platform pre-check is deliberately not schema-driven: `kesha install --diarize` on linux-x64 with no Engine on disk has nothing to validate against, so that check stays where it is and the platform pre-check reports `E_UNSUPPORTED_PLATFORM` (`assertPlatformCanInstall` in `src/engine-install.ts`, run by `installEngine` and by `kesha install` before anything else). Schema validation applies from the moment an Engine binary exists, and a gated flag on a build that lacks the gate is `E_INVALID_ARG`.
...
Compatibility window. `tts-e2e` runs the v3 CLI against the source-built Engine on every `rust/**` PR, so stage 1 keeps `--capabilities-json`, `--error-codes-json`, the `error [CODE]:` line, the `diarize:` prefix and the `KESHA_DEBUG_FD` sink whenever `KESHA_PROTOCOL` is unset; `KESHA_PROTOCOL=4` selects the event stream. Stage 2's first CLI PR sets that variable at spawn; the next Rust PR deletes the v3 renderer, the two flags and the sink, and cuts `v1.25.0-beta.2`, which becomes the pin. The capability-pact recorder and its fixtures switch to `describe` with that pin bump, because they record the published Engine.
