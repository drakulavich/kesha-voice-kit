...
- [ ] 3.3 Rust tests `error_codes_cli.rs`, `diarize_e2e.rs`, `kokoro_rate_e2e.rs`, `tts_smoke.rs`, `debug_ndjson_fd.rs` assert on events. Four of the five are done; `error_codes_cli.rs` keeps one `error [E_` assertion on purpose, because that is the v3 grammar the window still serves. `debug_ndjson_fd.rs` converts at 4.2 rather than now — it tests the `KESHA_DEBUG_FD` sink, which 2.5 keeps until then
...
- [ ] 4.2 Remove the `KESHA_PROTOCOL` window and cut `v1.25.0-beta.2`. Everything 1.3, 2.4 and 2.5 defer lands here:
  ...
  - the tests that pin the window, which are deletions rather than edits: `describe_cli.rs`'s two `…_still_answers_during_the_window` cases, `debug_events_v4.rs::v3_debug_trace_is_unchanged` (it asserts the `[debug/engine +` prose), `capabilities.rs`'s `assert_eq!(get_capabilities().protocol_version, 3)`, and the eleven `Mode::V3` cases in `record.rs`, `models/progress.rs` and `models/download.rs`
  ...
  - `rust/tests/debug_ndjson_fd.rs` is the exception: it is **converted, not deleted**. The engine-contract spec's technical note says it "becomes a test of `debug` events on stderr", and 3.3 owns that requirement — the conversion simply cannot happen before the sink goes, which is here
