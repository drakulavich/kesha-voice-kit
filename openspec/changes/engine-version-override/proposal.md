...
downloads — `src/engine-install.ts:200` and `:439` build the release URL from it and
nothing resolves a floating "latest". That makes the pin a switch rather than a hint:
...
committing a throwaway build points all of the above at it. But the alpha Engine channel
(#685 group 7) exists precisely so a change can be *tried* before it is committed, and
today there is no way to install a specific Engine version without editing the pin.
...
- `kesha doctor` reports an installed Engine that differs from the pin as a distinct,
  named state rather than as health or as silence.
...
- `diagnostics`: `kesha doctor` gains a reported state for "installed Engine does not match
  the pin", which today is invisible.
...
- `src/engine-install.ts` — `downloadEngine` takes the requested version rather than
  ...
  follow it.
- `src/doctor.ts` — reports pin-vs-installed drift.
