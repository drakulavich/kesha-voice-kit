...
`kesha install` SHALL take a lock on the Engine directory before writing to it, so that two installs sharing one Model cache never overwrite each other, and SHALL wait for a live holder rather than fail. A lock whose owner is dead, whose owner has held it past the stale ceiling, or whose owner record cannot be read SHALL be cleared by the next waiter within one poll interval rather than waited out. A waiter that outlasts the wait ceiling SHALL fail with `E_INSTALL_RACE`, naming the holder when it can and the lock path to delete in every case.
...
#### Scenario: The lock's owner record is unreadable
...
> record, or the owner file's token with no record when the file does not parse, or null
> when there is no owner file; `clearLock` unlinks the owner by its exact name and then
