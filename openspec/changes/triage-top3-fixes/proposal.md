...
- **Install lock (#1224):** `src/install-lock.ts::readOwner` distinguishes "no owner file" from "an owner file that does not parse". An unparseable owner is treated as stale: its file is unlinked by the exact name it has, then the directory is removed, so a foreign or corrupt owner costs one poll rather than the stale ceiling. A lock directory holding something that is not an owner file still cannot be cleared and still ends in `E_INSTALL_RACE` after the wait, as today.
...
- `installation`: adds the requirement that concurrent installs sharing one Engine directory are serialised by a lock, that a lock whose owner is dead, expired **or unreadable** is cleared by the next waiter rather than waited out, and that giving up is `E_INSTALL_RACE`. The serialisation itself has been the behaviour since #997 but was never a stated requirement; the unreadable-owner clause is the new behaviour.
