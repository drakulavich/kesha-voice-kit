...
- The abort path (`opts.signal`) is reachable from the Core API and from an
  MCP cancellation; no CLI flag or timeout wires an `AbortSignal` into a run.
  The Raycast extension gets its cancellation by killing the CLI process
  instead, which lands on the signal path above rather than this one.
- `waitForPendingSignalCleanup` is consulted by the main transcription command,
  `kesha install` and `kesha record`; `kesha say` still relies on the
  handler's own `process.exit`, so whether its command-owned exit can race
