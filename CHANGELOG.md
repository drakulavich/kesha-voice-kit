...
### Changed
- **BREAKING (`kesha --json` / `kesha --toon` on a batch where every file failed):** stdout is now empty instead of `[]`. A well-formed empty array is a success-shaped payload emitted on failure — a consumer that ignores the exit code reads it as "ran fine, nothing found", which is how a diarization failure surfaced as a silent empty transcript in [#768](https://github.com/drakulavich/kesha-voice-kit/issues/768). Partial failures are unchanged: files that succeeded still print as the results array. `--json --include-errors` is unchanged too and remains the way to get failures on stdout — it still prints `{ "results": [], "errors": [...] }`, and it is the only such channel, since `--include-errors` is rejected with `--toon`. `--format transcript` and `--verbose` stop emitting a lone newline on this path, so every format now leaves stdout byte-empty where plain text already did ([#773](https://github.com/drakulavich/kesha-voice-kit/issues/773), [#837](https://github.com/drakulavich/kesha-voice-kit/pull/837)).
