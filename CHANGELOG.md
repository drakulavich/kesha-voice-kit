...
### Changed
- **`kesha --json` / `kesha --toon` `textLanguage` gains a `source` field** (`"engine"` or `"tinyld"`), and the `tinyld` fallback now reports its own score instead of a hardcoded `confidence: 0`. Text detection runs in the Engine only on macOS; everywhere else the CLI falls back to `tinyld`, and that fallback used to be indistinguishable in the payload from the Engine genuinely returning a 0-confidence detection — so a consumer gating on `confidence > 0` silently discarded **every** Linux/Windows text detection. The two confidences are **not on the same scale** (`NLLanguageRecognizer`'s probability vs `tinyld`'s n-gram accuracy): read `source` first, then apply a per-source policy. Additive for anyone reading `code`/`confidence`; `audioLanguage` is unchanged and carries no `source` because it has only one. A side effect: `--format transcript` and `--verbose` on non-macOS now print the fallback's real confidence where they printed `0.00` or nothing ([#941](https://github.com/drakulavich/kesha-voice-kit/issues/941)).
- **BREAKING (`kesha --json` / `kesha --toon` on a batch where every file failed):** stdout is now empty instead of `[]`. A well-formed empty array is a success-shaped payload emitted on failure — a consumer that ignores the exit code reads it as "ran fine, nothing found", which is how a diarization failure surfaced as a silent empty transcript in [#768](https://github.com/drakulavich/kesha-voice-kit/issues/768). Partial failures are unchanged: files that succeeded still print as the results array. `--include-errors` is unchanged too and remains the way to get failures on stdout — it still prints `{ "results": [], "errors": [...] }`. `--format transcript` and `--verbose` stop emitting a lone newline on this path, so every format now leaves stdout byte-empty where plain text already did ([#773](https://github.com/drakulavich/kesha-voice-kit/issues/773), [#837](https://github.com/drakulavich/kesha-voice-kit/pull/837)).
...
### Added
- **`kesha --toon --include-errors`:** the error envelope now composes with TOON, not just JSON. Previously `--include-errors` was rejected with exit 2 unless `--json` was given, which left the format recommended for piping into an LLM with no structured way to see per-file failures — after [#837](https://github.com/drakulavich/kesha-voice-kit/pull/837) an all-failed `--toon` batch is empty stdout plus an exit code, and an agent reading only stdout sees nothing at all. `--toon --include-errors` now emits the same `{ results, errors }` envelope `--json --include-errors` does, encoded as TOON and round-tripping through `decode()` to identical data. Plain-text and `--format transcript` output still reject the flag: they have no structured channel to put the errors in ([#839](https://github.com/drakulavich/kesha-voice-kit/issues/839)).
