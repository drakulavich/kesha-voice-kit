...
### Added
- **Russian abbreviation auto-expansion for `ru-vosk-*` voices.** Detects 2–5 letter all-uppercase Cyrillic tokens and reads them letter-by-letter via the embedded letter-name table. The rule fires when the token cannot be pronounced as a natural Russian syllable — length ≤ 2 (ИП → "и пэ"), 0 vowels (ФСБ → "фэ эс бэ"), 2+ consecutive vowels (ОАЭ → "о а э"), or 2+ consecutive consonants (США → "сэ шэ а"). Tokens with strict CVC/CVCV alternation pass through (ВОЗ → "воз", НАТО → "нато", ОПЕК → "опек"). Letter-name forms tuned to user-validated pronunciation: Ф → "фэ", Ш → "шэ", Л → "эл", С → "сэ" at start / "эс" elsewhere. Stop-list for common short words (ОН, МЫ, КАК, ЧТО, …) prevents false positives. Tokens containing Ъ/Ь are passed through literally. Opt-out via `--no-expand-abbrev` flag. Closes [#232](https://github.com/drakulavich/kesha-voice-kit/issues/232).
