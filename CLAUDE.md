...
A guard is only a guard if removing it goes red. `just mutate [--timeout S] [--occurrences N] <file> <find> <replace> <test>` proves that in seconds — it refuses (exit 2) when the text does not occur or occurs more often than `--occurrences` says, runs the command once on the untouched file and demands green, mutates, restores from memory and from a `<file>.mutate-orig` sidecar even on SIGTERM, exits 3 (`NOT A VALID RUN`) when the baseline is red or a run outlives its timeout (default 600 s), and exits 0 only when the mutation was *caught*. A hand-rolled `perl -0pi` that matches nothing exits 0 and reads as "the pin is useless" (#1075); a command that cannot start used to read as PINNED (#1155), and a second match in a cleanup path used to hang forever (#1211).
