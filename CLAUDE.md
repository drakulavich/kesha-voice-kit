...
- Always nextest for the suite — the only sanctioned plain `cargo test` calls are `--doc` and the pin-bump's `models::manifest::manifest_tests`; always `--all-targets`, or CI catches `#[cfg(test)]` dead code you didn't.
