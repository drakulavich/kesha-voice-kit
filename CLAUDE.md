...
Every flag-carrying engine argv on a parsed path — `runEngine` callers in `src/engine.ts`, `say()` and `listVoiceIds()` in `src/synth.ts`, `validateInstallRequest()` in `src/engine-install.ts` — goes through `validateArgv` (`src/engine/describe.ts`) against `kesha-engine describe` before the spawn; `record` validates through `validateRecordRequest` but still inherits stderr. Only `record` still spawns on protocol 3. A flag the schema does not list for that subcommand, a flag whose gate the build lacks, a missing `requires` or a present `conflicts` is `E_INVALID_ARG` with no subprocess; `whenUngated: drop` rows are omitted with one warning. Do not add a new hand-written feature check; add a row to `gate_rows()` in `rust/src/protocol/describe.rs` (and its mirror in `tests/helpers/fake-engine.ts`, pinned by `describe-template.test.ts`).
